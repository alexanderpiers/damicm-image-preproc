...
def computeClusterVarianceRatio(image, npixels=100, ntrials=10000):
    ...
    clusterVarMed, clusterVarDistribution = ps.imageNoiseVariance(
        image, npixels, ntrials=ntrials
    )
    clusterVarMad = scipy.stats.median_absolute_deviation(
        clusterVarDistribution, axis=None
    )

    # Histogram the distribution around the central point. Fit to this central distribution
    bins = np.linspace(
        clusterVarMed - 5 * clusterVarMad, clusterVarMed + 5 * clusterVarMad, 200
    )
    ...
    gausfunc = lambda x, *p: p[0] * np.exp(-(x - p[1]) ** 2 / (2 * p[2] ** 2))
    paramGuess = [
        len(clusterVarDistribution) / np.sqrt(2 * np.pi * clusterVarMad ** 2),
        clusterVarMed,
        clusterVarMad,
    ]
    paramOpt, paramCov = optimize.curve_fit(
        gausfunc, clusterBinCenters, clusterH, p0=paramGuess
    )

    # Compute the ratio between the tails of the fit and data
    tailLoc = paramOpt[1] + 3 * paramOpt[2]
    tailRatio = np.sum(clusterVarDistribution > tailLoc) / (
        ntrials
        * (1 - scipy.stats.norm.cdf(tailLoc, loc=paramOpt[1], scale=paramOpt[2]))
    )
    return tailRatio
